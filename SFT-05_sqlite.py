...
def db_generate(db_name, journal_mode, encoding, page_size, rows, sys_exit):
    global id_list, top_1000_word_list
    global con, cur
    
    data = []
    id_list = random.sample(range(1000,10000),rows)
    ...
    cur.execute("CREATE TABLE users (UserID INT PRIMARY KEY, UserName TEXT, Phone TEXT, ProfileImage BLOB, Sex BOOL, UpdateTime REAL)")
    sql = "INSERT INTO users (UserID, UserName, Phone, ProfileImage, Sex, UpdateTime) VALUES (?, ?, ?, ?, ?, ?)"
    cur.executemany(sql, data)
    con.commit()

def data_rand_generate():
    UserID = id_list.pop()
    UserName = ' '.join(random.sample(top_1000_word_list,1))
    Phone = '010-' + format(random.randint(0000,9999), '04d') + '-' + format(random.randint(0000,9999), '04d')
    ProfileImage = blob_rand_generate(1) # seed = 1
    Sex = random.randint(0,1)
    UpdateTime = date_rand_generate() # datetime.fromtimestamp()

    return UserID, UserName, Phone, ProfileImage, Sex, UpdateTime
...
def main():
    #SFT-05.sqlite
    db_generate('SFT-05.sqlite', 'wal', 'UTF8', 4096, 100, 0)
