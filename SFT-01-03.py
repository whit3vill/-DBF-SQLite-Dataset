...
def data_rand_generate():
    ...
    Media = blob_rand_generate(1) # seed = 1
...
def main():
    ...
    if n == 1: # SFT-01-UTF8-WAL.sqlite
        ...
        con = sqlite3.connect('SFT-01-UTF8-WAL.sqlite', check_same_thread=False)
        cur = con.cursor()
        cur.execute("SELECT * FROM messages")
        sys.exit("Error!!!")
        
    elif n == 2: # SFT-01-UTF16BE-PERSIST.sqlite
        db_generate('SFT-01-UTF16BE-PERSIST.sqlite', 'persist', 'UTF16BE', 1024, 100)
    
    elif n == 3: # SFT-01-UTF16LE-OFF.sqlite
        db_generate('SFT-01-UTF16LE-OFF.sqlite', 'OFF', 'UTF16LE', 8192, 100)

    elif n == 4: # SFT-03-PERSIST.sqlite
